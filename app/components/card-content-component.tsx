...
import {TagSelectionOnForm} from "./edit/tag-selection-on-form";
...
export default function CardContentComponent({
  ...
  const fullCitation = useMemo(
    () => (currentCard ? buildFullCitation(currentCard) : ""),
    [currentCard]
  );
  const fragment = useMemo(() => currentCard?.fragment ?? "/", [currentCard]);
  const score = useMemo(() => currentCard?.score ?? "", [currentCard]);
  const tags = useMemo(() => currentCard?.tags ?? [], [currentCard]);
  ...
  return (
    <>
      ...
      <div className="absolute left-0 bottom-0 p-2 mt-2">
        <TagSelectionOnForm tags={tags} />
      </div>
    </>
  );
}
